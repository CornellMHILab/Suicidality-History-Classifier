...
NLP_OUTPUT_FILE = BASELINE_DIR + "/fsh_gs_data.csv"
...
# specify classifier model Gatatron
...
def get_target_pipeline():
    ...
    target_matcher.add(fsh_target_rules)
    return nlp
...
def get_file_data(file_name, size, start):
    ...
    for index, row in batch_df.iterrows():
        document = (row['id'], row['note_text'], row['fsh_label'])
...
def create_baseline_data(df):
    ...
    for i in range(df.shape[0]):
        ...
        input_sent = df.iloc[i]['FAMILY_SUICIDE_HISTORY']
        input_sent = re.sub(r"[\n\r\t]", ' ', input_sent)
...
def create_prompt_data(df):
    ...
    for i in range(df.shape[0]):
        suicide_id.append(df.iloc[i]['id'])
        if isinstance(df.iloc[i]['FAMILY_SUICIDE_HISTORY'], float):
            suicide_input_sent = str(df.iloc[i]['FAMILY_SUICIDE_HISTORY'])
        else:
            suicide_input_sent = df.iloc[i]['FAMILY_SUICIDE_HISTORY']
...
class ModelClass(torch.nn.Module):
    def __init__(self):
        ...
        self.classifier1 = torch.nn.Linear(1024, OUT_DIM1)
...
if __name__ == '__main__':
    ...
    if DATA_SOURCE == 'FILE':
        ...
        if len(extracted_data) > 0:
            ...
            df = pd.DataFrame(extracted_data, columns=['id', 'FAMILY_SUICIDE_HISTORY', 'label'], dtype=float)
